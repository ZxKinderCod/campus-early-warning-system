...
from .path_utils import resolve_contents_path
...
class AIJLAgentProxyHandler(WebSocketHandler):
    ...
    async def _check_agent_mode_timeout(self, request_id: str, tool_name: str, timeout_seconds: float):
        """Timeout watchdog for frontend-interactive tools."""
        await asyncio.sleep(timeout_seconds)

        # If the request_id is still tracked, no response came back from the frontend.
        if hasattr(self, '_pending_frontend_requests') and request_id in self._pending_frontend_requests:
            print(f"[PROXY] Frontend did not respond to {tool_name} within {timeout_seconds} seconds")

            error_result = (
                f"Error: No response from frontend within {timeout_seconds} seconds. "
                "Check kernel availability or notebook state, then retry."
            )
    ...
    async def _handle_tool_call_request(self, data):
        ...
        try:
            ...
            if tool_meta.location == "frontend":
                ...
                normalized_args, error_message = self._normalize_frontend_tool_args(tool_args)
                if error_message:
                    await self._send_to_remote_backend({
                        "type": "tool_call_result",
                        "request_id": request_id,
                ...
                if success:
                    ...
                    if tool_meta.timeout_on_pending:
                        ...
                        timeout_seconds = tool_meta.frontend_timeout_seconds or 10.0
                        asyncio.create_task(
                            self._check_agent_mode_timeout(request_id, tool_name, timeout_seconds)
                        )
                else:
    ...
    def _normalize_frontend_tool_args(self, tool_args: dict) -> tuple[dict, str | None]:
    ...
    def _resolve_contents_path(self, file_path: str) -> tuple[str, str | None]:
        ...
        return resolve_contents_path(file_path, root_dir)
